...
test("keeps the invitation and response workflow contract in source", async () => {
  ...
  assert.match(page, /Choose one or more/);
  assert.match(page, /Let them choose/);
  assert.match(preview, /activity-\$\{primaryActivity\.id\}/);
  assert.match(preview, /preview-activity-list/);
  assert.match(preview, /You choose what works/);
  ...
  assert.match(css, /\.time-mode-switch/);
  assert.match(css, /\.response-activity-picker/);
  ...
  assert.match(invitationTypes, /activities: ActivityId\[\]/);
  assert.match(invitationTypes, /timeMode: TimeMode/);
